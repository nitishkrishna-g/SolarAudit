...
export function ProductCard({ product }: ProductCardProps) {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center justify-center py-6 bg-white/5">
            ...
            <div className="p-4 flex flex-col flex-grow relative z-10">
                <div className="mb-2">
                    <span className={`text-xs font-medium uppercase tracking-wider ${meta.accent}`}>{product.category}</span>
                ...
                {product.rating && (
                    <div className="flex items-center gap-1 mb-3">
                        ...
                        <span className="text-xs text-slate-500 dark:text-slate-400">
                            {product.rating.toFixed(1)} ({(product.reviews ?? 0).toLocaleString()})
                        </span>
                    </div>
                )}

                <p className="text-sm text-slate-600 dark:text-slate-400 mb-4 line-clamp-2 flex-grow italic leading-snug">
                    {product.description}
                </p>

                {/* CTA */}
                <div className="mt-auto">
                    <span className="inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-amber-500 to-orange-500 group-hover:from-amber-400 group-hover:to-orange-400 text-white font-bold py-2.5 px-4 rounded-xl transition-all shadow-lg shadow-amber-500/25 group-hover:shadow-amber-500/40 text-sm">
